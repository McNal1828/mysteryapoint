...
export default function Home() {
    ...
    useEffect(() => {
        ...
        data.forEach(({ backnumber, attack, normal, defense, name }) => {
            ...
            result[backnumber].attack += attack.reduce(
                (sum, item) => sum + (item.point || 0),
                0
            );
            result[backnumber].normal += normal.reduce(
                (sum, item) => sum + (item.point || 0),
                0
            );
            result[backnumber].defense += defense.reduce(
                (sum, item) => sum + (item.point || 0),
                0
            );
            result[backnumber].total =
                result[backnumber].attack +
                result[backnumber].normal +
                result[backnumber].defense;
        });
        settotal(
    ...
    useEffect(() => {
        ...
        recentdate.forEach((date) => {
            ...
            data.filter((item) => item.date === date).forEach(
                ({ backnumber, attack, normal, defense, name }) => {
                    if (!result[date][backnumber]) {
                        result[date][backnumber] = {
                            name,
                            attack: 0,
                            normal: 0,
                            defense: 0,
                    ...
                    result[date][backnumber].attack += attack.reduce(
                        (sum, item) => sum + (item.point || 0),
                        0
                    );
                    result[date][backnumber].normal += normal.reduce(
                        (sum, item) => sum + (item.point || 0),
                        0
                    );
                    result[date][backnumber].defense += defense.reduce(
                        (sum, item) => sum + (item.point || 0),
                        0
                    );
                    result[date][backnumber].total =
                        result[date][backnumber].attack +
                        result[date][backnumber].normal +
                        result[date][backnumber].defense;
                }
            );
        });
        setrecent(result);
    ...
    useEffect(() => {
        ...
        for (let backnumber of playerBacknumberList) {
            ...
            for (let playerdata of playerDatas) {
                playerdata.total = 0;
                playerdata.total += playerdata.attack.reduce(
                    (sum, item) => sum + (item.point || 0),
                    0
                );
                playerdata.total += playerdata.defense.reduce(
                    (sum, item) => sum + (item.point || 0),
                    0
                );
                playerdata.total += playerdata.normal.reduce(
                    (sum, item) => sum + (item.point || 0),
                    0
                );
            ...
            for (let rdate of recentdate) {
                tempresult.push(
                    playerDatas.filter((item) => item.date === rdate)[0] || {}
                );
            }
            ...
            <dialog open={ismodalopen}>
                <article>
                    <header>
                        ...
                        <h3>
                            {cdata.name}{" "}
                            {new Date(cdata.date).getUTCMonth() + 1}/
                            {new Date(cdata.date).getUTCDate()} 기록
                        </h3>
                    </header>
                    <main>
                        <p>
